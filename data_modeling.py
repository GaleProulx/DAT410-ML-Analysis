...
#Finn
def build_model(nn=16, nl=5, loss="mean_squared_error", optimizer="rmsprop", activation="relu"):
    model = Sequential()
...
def hyperparameter_tuning(X_train, y_train):
    ...
    params = {'activation': ['relu', 'tanh', 'elu', 'softmax', 'selu'], 'batch_size': [24, 32, 128, 256],
              'epochs': [25, 50, 100, 200], 'nl': [1, 2, 5, 9],
              'nn': [20, 60, 128, 256], 'loss': ['mean_squared_error', 'huber_loss', 'binary_crossentropy',
                                                 'categorical_crossentropy'],
              'optimizer': ['SGD', 'RMSprop', 'Adagrad', 'Adam', 'Adamax']}

    random_search = RandomizedSearchCV(model, param_distributions=params, cv=3, n_jobs=-1, verbose=1)
